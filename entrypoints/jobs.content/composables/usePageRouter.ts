...
const routeConfigs = [
    {
        externalPath: '/',
        internalName: TEST,
        parser: jobListTableParser
    },
];

function determineRouteNameFromURL(url: string): string {
    return TEST;
}
...
export function usePageRouter() {
    ...
    function parseAndRedirect(htmlContent: string, redirect: boolean = true, targetRouteName?: string) {
        ...
        if (redirect)
        {
            pagePropsStore.setPageProps(TEST, {jobList: value})
            void router.push({
                name: routeName,
            })
        }
